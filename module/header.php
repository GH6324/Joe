...
<header class="joe_header <?php echo $this->is('post') ? 'current' : '' ?>">
				...
				<?php while ($category->next()) : ?>
					<?php if ($category->levels === 0) : ?>
						...
						<?php if (empty($children)) : ?>
							<a class="item <?php echo $this->is('category', $category->slug) ? 'active' : '' ?>" href="<?php $category->permalink(); ?>" title="<?= htmlentities($category->name) ?>"><?php $category->name(); ?></a>
						<?php else : ?>
	...
	<div class="joe_header__slideout">
		...
		<div class="joe_header__slideout-author">
			...
			<div class="info">
				<a class="link" href="<?php $this->options->JAside_Author_Link() ?>" target="_self" rel="noopener noreferrer nofollow"><?php $this->options->JAside_Author_Nick ? $this->options->JAside_Author_Nick() : ($this->authorId ? $this->author->screenName() : $this->user->screenName()); ?></a>
